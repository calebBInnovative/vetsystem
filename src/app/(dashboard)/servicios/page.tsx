...
import {
  useServicios, crearServicio, actualizarServicio,
  toggleServicioActivo, eliminarServicio, type ServicioInput,
} from '@/hooks/useServicios';
import { CATEGORIAS_SERVICIO, type CategoriaServicio, type ServicioLocal } from '@/types/servicio';
...
const CATEGORIAS_LIST = Object.entries(CATEGORIAS_SERVICIO) as [CategoriaServicio, { label: string; emoji: string; color: string }][];

// ─── Formulario inline (crear / editar) ──────────────────────────────────────

interface ServicioFormProps {
  inicial?: ServicioLocal;
...
function ServicioForm({ inicial, onGuardar, onCancelar }: ServicioFormProps) {
  ...
  const [categoria,  setCategoria]  = useState<CategoriaServicio>(inicial?.categoria ?? 'consulta');
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="sm:col-span-2">
          ...
          <input
            ...
            placeholder="Ej: Consulta General, Castración…"
...
function ServicioRow({ servicio, onEditar }: ServicioRowProps) {
  ...
  const cat = CATEGORIAS_SERVICIO[servicio.categoria];

  async function guardarPrecio() {
    const n = Number(precioTmp);
    if (n > 0 && n !== servicio.precio) {
      await actualizarServicio(servicio.id, { precio: n });
    }
...
export default function ServiciosPage() {
  ...
  const [editando,     setEditando]     = useState<ServicioLocal | null>(null);
  const [filtroCat,    setFiltroCat]    = useState<CategoriaServicio | 'todas'>('todas');
  ...
  const porCategoria = filtrados.reduce<Record<string, ServicioLocal[]>>((acc, s) => {
  ...
  async function handleEditar(input: ServicioInput) {
    if (!editando) return;
    await actualizarServicio(editando.id, input);
    setEditando(null);
  }
    ...
    <div className="space-y-5">
      ...
      {cargando ? (
        ...
        <div className="space-y-4">
          {Object.entries(porCategoria).map(([cat, items]) => {
            const catInfo = CATEGORIAS_SERVICIO[cat as CategoriaServicio];
            return (
      ...
      {servicios.length > 0 && (
        <p className="text-xs text-muted-foreground text-center">
          {servicios.filter((s) => s.activo).length} activos · {servicios.filter((s) => !s.activo).length} inactivos · {servicios.length} total
        </p>
      )}
    </div>
  );
