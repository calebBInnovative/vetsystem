...
const ESTADOS: { key: EstadoFactura; label: string; desc: string; emoji: string }[] = [
...
export function FacturaModal({ consulta, open, onGuardada }: FacturaModalProps) {
  ...
  const [estado, setEstado]           = useState<EstadoFactura>('pagada');
    ...
    <Dialog open={open} onOpenChange={() => {}}>
      ...
      >
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border">
          <DialogTitle className="text-lg font-bold">Factura / Recibo</DialogTitle>
          ...
          <div className="rounded-2xl border border-border overflow-hidden">
            ...
            {items.length === 0 ? (
              <div className="px-4 py-6 text-center text-sm text-muted-foreground">
                Sin productos ni servicios registrados
              </div>
            ) : (
                  ...
                  <div key={item.id} className="px-4 py-3 flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.esServicio ? 'Servicio' : 'Producto'} · {item.cantidad} × {fmt(item.precioUnitario)}
                      </p>
                    </div>
