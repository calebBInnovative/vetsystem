...
import { type CitaConPaciente, ESTADOS_CITA, TIPOS_CITA, type EstadoCita } from '@/types/agenda';
import { ESPECIES } from '@/types/paciente';
import { cambiarEstadoCita, eliminarCita } from '@/hooks/useAgenda';
...
export function CitaCard({ cita }: CitaCardProps) {
  ...
  const especie   = cita.especiePaciente ? ESPECIES[cita.especiePaciente as keyof typeof ESPECIES] : null;
  const siguientes = TRANSICIONES[cita.estado];

  const horaFin = calcularHoraFin(cita.horaInicio, cita.duracionMinutos);

  const handleCambioEstado = async (nuevoEstado: EstadoCita) => {
    setCambiandoEstado(true);
    try {
      await cambiarEstadoCita(cita.id, nuevoEstado);
      toast.success(`Cita ${ESTADOS_CITA[nuevoEstado].label.toLowerCase()}`);
    } catch {
            ...
            {siguientes.map((sig) => (
              <button
                ...
                className={cn(
                  ...
                  ESTADOS_CITA[sig].color
                )}
              >
                <span className={cn('w-1.5 h-1.5 rounded-full inline-block mr-1', ESTADOS_CITA[sig].punto)} />
                {ESTADOS_CITA[sig].label}
              </button>
            ))}
            <Button
              variant="ghost"
